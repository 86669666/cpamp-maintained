...
func TestManagerDataSnapshotProcessDeathHelper(t *testing.T) {
	phase := os.Getenv("CPA_MANAGER_SNAPSHOT_DEATH_PHASE")
	if phase == "" {
		t.Skip("helper process only")
	}
	index, err := strconv.Atoi(os.Getenv("CPA_MANAGER_SNAPSHOT_DEATH_INDEX"))
	if err != nil {
		t.Fatalf("parse death index: %v", err)
	}
	afterRestoreRenameFn = func(actualPhase string, actualIndex int) {
		if actualPhase == phase && actualIndex == index {
			os.Exit(86)
		}
	}
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err = Run(context.Background(), snapshotArgs(
		"restore",
		os.Getenv("CPA_MANAGER_SNAPSHOT_DEATH_DB"),
		os.Getenv("CPA_MANAGER_SNAPSHOT_DEATH_DATA_KEY"),
		os.Getenv("CPA_MANAGER_SNAPSHOT_DEATH_DIR"),
	), &stdout, &stderr)
	t.Fatalf("restore did not terminate at %s/%d: err=%v stdout=%s stderr=%s", phase, index, err, stdout.String(), stderr.String())
}

func TestRecoverAfterProcessDeathAtEveryRestoreRenameBoundary(t *testing.T) {
	if os.PathSeparator == '\\' {
		t.Skip("subprocess exit semantics are POSIX-only")
	}
	for _, phase := range []string{"live", "snapshot"} {
		for index := range snapshotFiles {
			t.Run(fmt.Sprintf("%s-%d", phase, index), func(t *testing.T) {
				dataDir := t.TempDir()
				dbPath := filepath.Join(dataDir, "usage.sqlite")
				dataKeyPath := filepath.Join(dataDir, "data.key")
				snapshotDir := filepath.Join(dataDir, ".cpamp-manager-snapshot-old")
				writeCompleteRestoreSet(t, dbPath, dataKeyPath, "old")
				runSnapshotCommand(t, "create", dbPath, dataKeyPath, snapshotDir)
				writeCompleteRestoreSet(t, dbPath, dataKeyPath, "new")

				cmd := exec.Command(os.Args[0], "-test.run=^TestManagerDataSnapshotProcessDeathHelper$")
				cmd.Env = append(os.Environ(),
					"CPA_MANAGER_SNAPSHOT_DEATH_PHASE="+phase,
					"CPA_MANAGER_SNAPSHOT_DEATH_INDEX="+strconv.Itoa(index),
					"CPA_MANAGER_SNAPSHOT_DEATH_DB="+dbPath,
					"CPA_MANAGER_SNAPSHOT_DEATH_DATA_KEY="+dataKeyPath,
					"CPA_MANAGER_SNAPSHOT_DEATH_DIR="+snapshotDir,
				)
				output, err := cmd.CombinedOutput()
				var exitErr *exec.ExitError
				if !errors.As(err, &exitErr) || exitErr.ExitCode() != 86 {
					t.Fatalf("death helper exit=%v output=%s", err, output)
				}
				if err := Recover(dbPath, dataKeyPath); err != nil {
					t.Fatalf("recover interrupted restore: %v", err)
				}
				requireCompleteRestoreSet(t, dbPath, dataKeyPath, "new")
				requireNoRestoreTempFiles(t, dataDir)
				if _, err := os.Lstat(restoreIntentPath(dbPath)); !os.IsNotExist(err) {
					t.Fatalf("restore intent remains after recovery: %v", err)
				}
			})
		}
	}
...
func TestCreateRecoversInterruptedRestoreBeforeSnapshotting(t *testing.T) {
	dataDir := t.TempDir()
	dbPath := filepath.Join(dataDir, "usage.sqlite")
	dataKeyPath := filepath.Join(dataDir, "data.key")
	oldSnapshotDir := filepath.Join(dataDir, ".cpamp-manager-snapshot-old")
	newSnapshotDir := filepath.Join(dataDir, ".cpamp-manager-snapshot-new")
	writeCompleteRestoreSet(t, dbPath, dataKeyPath, "old")
	runSnapshotCommand(t, "create", dbPath, dataKeyPath, oldSnapshotDir)
	writeCompleteRestoreSet(t, dbPath, dataKeyPath, "new")

	previous := afterRestoreRenameFn
	afterRestoreRenameFn = func(phase string, index int) {
		if phase == "snapshot" && index == 1 {
			panic("simulated process death")
		}
	}
	func() {
		defer func() { _ = recover() }()
		_, _ = restoreWithWarnings(context.Background(), dbPath, dataKeyPath, oldSnapshotDir)
	}()
	afterRestoreRenameFn = previous

	runSnapshotCommand(t, "create", dbPath, dataKeyPath, newSnapshotDir)
	requireCompleteRestoreSet(t, dbPath, dataKeyPath, "new")
	if _, err := os.Lstat(restoreIntentPath(dbPath)); !os.IsNotExist(err) {
		t.Fatalf("restore intent remains after create recovery: %v", err)
	}
}

func writeCompleteRestoreSet(t testing.TB, dbPath string, dataKeyPath string, suffix string) {
	t.Helper()
	writeTestFile(t, dbPath, "database-"+suffix, 0o600)
	writeTestFile(t, dbPath+"-wal", "wal-"+suffix, 0o600)
	writeTestFile(t, dbPath+"-shm", "shm-"+suffix, 0o600)
	writeTestFile(t, dbPath+"-journal", "journal-"+suffix, 0o600)
	writeTestFile(t, dataKeyPath, "key-"+suffix, 0o600)
}

func requireCompleteRestoreSet(t testing.TB, dbPath string, dataKeyPath string, suffix string) {
	t.Helper()
	requireTestFile(t, dbPath, "database-"+suffix)
	requireTestFile(t, dbPath+"-wal", "wal-"+suffix)
	requireTestFile(t, dbPath+"-shm", "shm-"+suffix)
	requireTestFile(t, dbPath+"-journal", "journal-"+suffix)
	requireTestFile(t, dataKeyPath, "key-"+suffix)
}
