// Adapted from https://github.com/gtxx3600/CPA2sub2API at commit 42ff785d5f3527e4050c02a0da51d4f7eb4f1180 (MIT).
...
function mapPlatformToProviderKey(platform) {
...
function extractApiKey(credentials, extra) {
  const sources = [
    credentials?.api_key,
    credentials?.["api-key"],
    credentials?.apiKey,
...
function convertSub2ApiApiKeyAccount(account) {
  const credentials = isPlainObject(account.credentials) ? account.credentials : {};
  const extra = getSub2ApiExtra(account);
  const apiKey = extractApiKey(credentials, extra);

  if (!apiKey) {
    throw new Error("credentials 中未找到 api_key、api-key、apiKey 或 key 字段");
  }

  const platform = String(account.platform || "").trim().toLowerCase();
  const providerKey = mapPlatformToProviderKey(platform);

  if (!providerKey) {
    throw new Error(`暂不支持 platform=${account.platform} 的 API Key 转换`);
  }

  const email = firstNonEmpty(extra.email, credentials.email, credentials.email_address);
  const accountName = firstNonEmpty(account.name, email);

  return {
    kind: "providerApiKey",
    sourceType: "apikey",
    providerKey,
    providerLabel: providerKey.replace("-api-key", "").replace(/^[a-z]/, (c) => c.toUpperCase()),
    platform,
    accountName,
    email,
    apiKey,
    entryLabel: accountName,
  };
}
...
function convertSub2ApiAccount(account, options = {}) {
  ...
  if (accountType === "apikey" || accountType === "api_key" || accountType === "api-key") {
    return convertSub2ApiApiKeyAccount(account);
  }
...
export function convertSub2ApiDocument(document, options = {}) {
  ...
  accounts.forEach((account, index) => {
    ...
    try {
      ...
      if (result.kind === "providerApiKey") {
        convertedApiKeys.push(result);
      } else {
  ...
  return { converted, convertedApiKeys, skipped };
}
...
export function buildMergedApiKeyConfig(convertedApiKeys) {
  const grouped = {};

  for (const item of convertedApiKeys) {
