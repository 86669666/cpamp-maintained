...
- Codex account history and Monitoring account windows reuse legacy data only when explicit account identity and exact credential provenance match; ambiguous, identity-less, or foreign records remain separated. `usage_events` remains the authoritative event ledger; bounded compatibility migrations may update normalized or response-metadata columns in place.
