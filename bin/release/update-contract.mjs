export const repository = 'https://github.com/86669666/cpamp-maintained';
const pattern =
...
export function parseVersion(tag) {
  ...
  const stage = !pre.length
    ...
    : pre.length === 2 && /^(beta|rc)$/.test(pre[0]) && /^\d+$/.test(pre[1])
      ? pre[0]
      : pre.length === 2 && pre[0] === 'maintained' && /^[1-9]\d*$/.test(pre[1])
        ? 'stable'
        : '';
...
export function validateInfo(info, tag) {
  ...
  if (
    ...
    guide.username ||
    guide.password ||
    !guide.pathname.startsWith('/86669666/cpamp-maintained/')
  )
    throw new Error('Invalid upgrade guide');
  if (
    info.distribution?.docker !== null ||
