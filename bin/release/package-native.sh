...
for target in "${targets[@]}"; do
  ...
  (
    ...
    CGO_ENABLED=0 GOOS="${goos}" GOARCH="${goarch}" go build -buildvcs=false -trimpath -ldflags "-s -w -X github.com/seakee/cpa-manager-plus/apps/manager-server/internal/buildinfo.Version=${version} -X github.com/seakee/cpa-manager-plus/apps/manager-server/internal/buildinfo.SourceCommit=$(git rev-parse HEAD)" -o "${package_dir}/${exe_name}" ./cmd/cpa-manager-plus
  )
